...
function getEmailHtml(user, verifyCode) {
  return `
      <!DOCTYPE html>
...
(async () => {
  // example data
  const user = { firstName: 'Test', email: 'test@example.com' };
  const verifyCode = '123456';

  const html = getEmailHtml(user, verifyCode);
  const outPath = path.join(__dirname, 'emailPreview.html');
  fs.writeFileSync(outPath, html, 'utf8');
  console.log(`✅ Preview written to ${outPath}. Open this in your browser to see the email.`);
})();
